...
class TenantThemeForm extends TPage
{
    ...
    public function __construct($param)
    {
        ...
        $this->form = new BootstrapFormBuilder('form_tenant_theme');
        $this->form->enctype = 'multipart/form-data';
        
        $id                 = new THidden('id');
        $tenant_id          = new THidden('tenant_id');
        ...
        $hover_effect->setValue('default');
        
        $this->logo_preview = new TImage('https://api.iconify.design/ph:image-square-duotone.svg?color=%23888888');
        $this->logo_preview->id = 'logo_preview_image';
        $this->logo_preview->style = 'max-width: 100px; max-height: 100px; border: 1px solid #ccc; border-radius: 4px; object-fit: contain; margin-left: 20px;';
        $logo_container = new THBox;
        $logo_container->add($url_logo);
        $logo_container->add($this->logo_preview);

        // --- Adiciona campos ao formulário ---
        $this->form->addFields([$id, $tenant_id]);
        $this->form->addFields([new TLabel('Logo da sua Loja')], [$logo_container]);
        ...
        $this->form->addAction('Salvar e Concluir', new TAction([$this, 'onSave']), 'fa:check-circle green')->class = 'btn btn-primary';
        $this->form->addAction('Voltar para o Login', new TAction(['LoginForm', 'onLoad']), 'fa:arrow-left')->class = 'btn btn-dark';
        
        $form_panel = TPanelGroup::pack('Personalize sua Loja', $this->form);
        $hbox->add($form_panel)->style = 'width: 50%; padding-right: 10px;';
        
        // CORRIGIDO: HTML do painel de pré-visualização foi adicionado
        ...
        // CORRIGIDO: CSS para os efeitos foi adicionado
        ...
        $style->add("
            ...
            .hover-effect-elevate:hover { transform: translateY(-4px); }
        ...
        // CORRIGIDO: Script para a interatividade foi adicionado
        ...
        $script->add("
            ...
            $('form[name=form_tenant_theme] input, form[name=form_tenant_theme] select').on('change input', updatePreview);
            $(document).ready(function(){ setTimeout(updatePreview, 150); });

            $('input[name=url_logo]').on('change', function(e) {
                if (e.target.files && e.target.files[0]) {
                    var reader = new FileReader();
                    reader.onload = function(event) {
                        $('#preview-logo').attr('src', event.target.result);
                        $('#logo_preview_image').attr('src', event.target.result);
                    };
    ...
    {
        try {
            // A página só funciona se receber um tenant_id
            if (isset($param['tenant_id'])) {
                TTransaction::open('permission');
                
                // Tenta carregar um tema existente para este tenant
                $theme = TenantTheme::where('tenant_id', '=', $param['tenant_id'])->first();
                $tenant = new Tenant($param['tenant_id']);
                
                // CENÁRIO 1: EDIÇÃO (usuário está voltando para editar)
                if ($theme && $tenant) {
                    ...
                    // Mostra o logo já salvo
                    if (!empty($tenant->url_logo) && file_exists($tenant->url_logo)) {
                        $this->logo_preview->src = $tenant->url_logo;
                        $script = new TElement('script');
                        $script->add(" $(document).ready(function(){ $('#preview-logo').attr('src', '{$tenant->url_logo}'); }); ");
                        parent::add($script);
                    }
                }
                // CENÁRIO 2: CRIAÇÃO (primeira visita do usuário)
                else {
                    // Limpa o formulário e apenas preenche o ID do tenant
                    $this->form->clear();
                    $data = new stdClass;
    ...
    public function onSave($param)
    {
        try {
            ...
            if (empty($data->tenant_id)) { throw new Exception('ID da loja não encontrado.'); }

            $tenant = new Tenant($data->tenant_id);
            $theme = new TenantTheme($data->id);
            ...
            new TMessage('info', 'Sua loja foi personalizada com sucesso!', $pos_action);

        } catch (Exception $e) { new TMessage('error', 'Ocorreu um erro: ' . $e->getMessage()); TTransaction::rollback(); }
    }
}