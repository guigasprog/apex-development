...
class TenantThemeForm extends TPage
{
    ...
    public function __construct($param)
    {
        ...
        // Carrega opções dinâmicas do banco de dados
        try {
            TTransaction::open('permission');
            $font_ui->addItems(CustomFont::getIndexedArray('name', 'label'));
            $hover_effect->addItems(CustomHoverEffect::getIndexedArray('name', 'label'));
            TTransaction::close();
        } catch (Exception $e) { new TMessage('error', 'Erro ao carregar opções de personalização: ' . $e->getMessage()); }

        // --- Adiciona campos ao formulário ---
        $this->form->addFields([$id, $tenant_id]);
        $this->form->addFields([new TLabel('Logo da sua Loja')], [$url_logo]);
    ...
    public function onLoad($param)
    {
            ...
            if (isset($param['tenant_id'])) {
                ...
                if ($theme && $tenant) { // Edição
                    ...
                    if (!empty($tenant->url_logo) && file_exists($tenant->url_logo)) {
                        ...
                        $script_logo = new TElement('script');
                        $script_logo->add(" $(document).ready(function(){ $('#preview-logo').attr('src', '{$tenant->url_logo}'); }); ");
                        parent::add($script_logo);
                    }
                }
                else { // Criação
                    $this->form->clear();
        ...
        } catch (Exception $e) { new TMessage('error', 'Erro ao carregar dados: ' . $e->getMessage()); TTransaction::rollback(); }
    }
    ...
    public function onSave($param)
    {
        try {
            ...
            $theme->tenant_id = $data->tenant_id;
            ...
            new TMessage('info', 'Sua loja foi personalizada com sucesso! Faça o login para começar.', $pos_action);
    ...
    private function injectDynamicAssets()
    {
        ...
        $script->add("
            function updateColorOptions() {
                ...
                if (typeof savedPrimaryColor !== 'undefined') { savedPrimaryColor = undefined; }
                if (typeof savedSecondaryColor !== 'undefined') { savedSecondaryColor = undefined; }
